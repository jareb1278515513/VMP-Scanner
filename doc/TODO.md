...
- [x] M2 漏洞探测层可用（插件框架 + 3~5 个插件）
- [x] M3 风险评估与报告闭环（评分 + 输出）
- [x] M4 表现层可用（Web 可视化报告）
- [x] M5 稳定性与工程化完善（测试、日志、文档）
...
- [x] CLI 输出实时进度与总结统计
- [x] 生成 JSON 报告（便于后续可视化接入）
- [x] 生成 Markdown 报告（便于归档和汇报）
- [x] 增加执行元数据（时间、模式、工具版本、参数）

交付标准：
- [x] 报告包含资产、漏洞、风险、建议四大部分
- [x] 报告结构可被脚本再次读取处理
...
- [x] 补充字段映射表（Detection/Assessment -> Presentation）
...
- [x] 实现多维筛选（风险等级、插件类型、目标资产、状态码区间）
- [x] 实现全文搜索（URL、参数名、漏洞标题、插件名）
- [x] 实现视图联动（点击概览卡片可联动筛选结果）
- [x] 增加批量展开/折叠与快速定位（回到顶部、锚点跳转）

交付标准：
- [ ] 1000 条 Finding 以内筛选与搜索响应可接受（目标 < 500ms）
- [x] 常见分析路径可在 3 次点击内到达详情
...
- [ ] 使用统一的参数配置文件,可以不在cli中输入过多参数,类似快速运行脚本
- [ ] 补充 README 运行说明与示例命令
